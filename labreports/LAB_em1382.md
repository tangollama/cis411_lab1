...
I believe the best architecture for these two issues, based upon what we've been learning in class and on my own research would be a layered architecture. This choice would work best as it allows for more flexibility per layer, allowing for integration with 3rd party services on certain layers, and an API layer could be added for further integration. Another benefit would be the fact that since the layers are decoupled, a layer can be upgraded or switched out without affecting the entire system. An issue, however, would be if one of the layers went down and compromised the layers that are dependant upon it.
